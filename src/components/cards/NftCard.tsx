  ...
  return (
    <div className={`${BASE_CARD} ${loading && `animate-pulse`}`}>
      ...
      <div
        ...
        onClick={() =>
          (window.location.href = `https://www.mintbase.io/thing/${nftInfo.thingId}`)
        }
      >
        <div className="h-full w-full rounded bg-mb-red object-fit">
          <img className="h-full rounded" src={nftInfo.nftImg} />
        </div>
      </div>
      <div className="flex flex-row justify-between text-black dark:text-white mt-12">
        <div className="big-p">{nftInfo.title}</div>
        <div className="big-p">{nftInfo.price}</div>
      </div>
      <div className="flex flex-row justify-between text-gray-200 mt-12">
        <div
          ...
          onClick={() =>
            (window.location.href = `https://www.mintbase.io/human/${nftInfo.ownerId}`)
          }
        >
          <img className="h-full rounded-full" src={nftInfo.ownerImg} />
        </div>
        <div className="med-p flex flex-row items-center text-black dark:text-white space-x-8">
          <div className="med-p">
            {nftInfo.availableEditions}/{nftInfo.totalEditions}
          </div>
          <div>
